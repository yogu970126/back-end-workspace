...
public class C_Sorting {
	...
	public void method3() {
		...
		Arrays.sort(score);
		System.out.printf("최소값 : %d, 최대값 : %d", score[0], score[score.length - 1]);
	}

	/*
	 * 선택정렬 (selection sort) - 가장 작은 데이터를 선택해 맨 앞에 있는 데이터와 바꾸고, - 그다음 작은 데이터를 선택해
	 * 앞아서 두 번째 데이터와 바꾸는 과정을 반복 - 배열을 전부 탐색하여 최소값을 고르고 왼쪽부터 채워나가는 방식의 정렬 - 데이터 양이 적을
	 * 때 좋음, 하지만 100개 이상인 경우 급격한 속도 저하
	 */
	public void method4() {
		int[] arr = { 7, 5, 9, 0, 3, 1, 6, 2, 4, 8 };
	 ...
	 * 
	 * 		7, 5, 9, 0, 3, 1, 6, 2, 4, 8
	 * 		5, 7, 0, 3, 1, 6, 2, 4, 8, 9 -> 가장 끝에 해당하는 9 (앞에 두 개 체인지)
