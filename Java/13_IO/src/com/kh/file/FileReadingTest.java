...
//		f.method1();
	...
	public void fileSave() {
		...
		try (FileWriter fw = new FileWriter(fileName, true)) { // true는 파일에 이어서 데이터를 추가
			fw.write("IO 재밌나요?\n");
			fw.write("아닌가요?\n");
	...
	public void fileRead() {
		...
		try (FileReader fr = new FileReader(fileName)) {
			...
			while ((data = fr.read()) != -1) {
				System.out.print((char) data); // 숫자로 읽어오기 때문에 문자로 형변환해주면 된다
			}
	...
	public void method1() {
		try (
				FileReader fr = new FileReader(fileName); // 기반 스트림
